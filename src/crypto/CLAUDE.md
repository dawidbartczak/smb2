...
- **Owned receive buffers need no payload clone for verification.** The crate-private `verify_signature_in_place` follows MS-SMB2 3.1.5.1 by saving the 16-byte signature, zeroing that field, computing over the full sub-frame (including compound padding), then comparing. It restores the received signature before propagating either a key/computation error or a mismatch, preserving exact wire bytes for callers and preauth hashing. The public immutable `verify_signature` keeps its original API by copying then using this same implementation. Algorithm/key/nonce selection is unchanged. Spec: https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-smb2/5b570c0b-0854-4324-8fb5-d410692dde3e
