...
- **Credits are reserved on send, never on receipt.** `CreditPool` (`credits.rs`) owns one connection-wide counter; `Inner::reserve_credits` takes the charge before the bytes go out, and only a `CreditResponse` grant puts credits back. Accounting on the response instead makes every in-flight request invisible, which is how concurrent streams over one connection out-spend the server's window and get themselves silently cut off.
- **Standalone CLOSE has its own FIFO Control queue; everything else is FIFO Data.** After each grant/refund, every fitting Control head is served before the Data head. Data never bypasses Data and credits are never partially reserved. This is the MS-SMB2 § 3.2.4.1.3 escape from credit head-of-line blocking: a 1-credit CLOSE may run while a multi-credit READ at the Data head waits for its full charge. ❌ Do not collapse this back to a fair `Semaphore::acquire_many`: fairness across unlike charges can strand the handle-closing request behind a READ the current window cannot fund. CANCEL still spends no credit; ECHO is opportunistic Data and therefore cannot bypass a queued CLOSE; every compound is Data even when it contains CLOSE.
...
Same owned-`Connection` + `Arc<Tree>` shape as `FileWriter`, so it's `'static`. `read_at` takes `&self` (no shared cursor) and issues `execute_with_credits` READs, splitting a range larger than `MaxReadSize` into consecutive wire reads and reassembling. It clamps to the size seen at open, so a read at/after EOF returns empty and a straddling read is short — never an error. `close()` consumes `self` (read-after-close is a compile error); `close_with_fingerprint_bounded(CloseDeadlines)` separately bounds credit admission and the response after `mark_sent`, so callers can distinguish a CLOSE that never reached the wire from a server that did not answer it. Timeout/transport loss never triggers a second CLOSE; only a protocol rejection on a still-live connection gets best-effort plain CLOSE fallback. Like the other stream handles, `Drop` can't CLOSE (no async drop) and only logs a debug warning, so a dropped-without-close reader leaks the handle until session teardown. Pinned by the `stream.rs` `file_reader_*` and `bounded_close_*` mock tests and the `guest_file_reader_positioned_reads` Docker test.
...
- **The receiver holds `Weak<Inner>` across socket reads.** It upgrades only to process a received frame or transport error. Holding an `Arc` while waiting created a cycle through `Inner.receiver_task`, so cancelled handshakes leaked both socket halves. Dropping one public clone still preserves the connection; dropping the final owner lets `Inner::drop` abort the plumbing. NEGOTIATE also finishes through `await_response`, so its silence obeys the same configured deadline as other requests.
